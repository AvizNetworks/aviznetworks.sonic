...
version_added: 1.0.0
notes:
- Tested against Enterprise SONiC Distribution by Aviz Technologies.
...
description:
  - Runs commands on remote devices running Enterprise SONiC Distribution
    by Aviz Technologies. Sends arbitrary commands to an Enterprise SONiC node and
  ...
  - This module does not support running commands in configuration mode.
    To configure SONiC devices, use M(delles.sonic_config).
options:
  commands:
    description:
...
from ansible_collections.aviznetworks.sonic_fmcli.plugins.module_utils.network.sonic.configs.vlan import VlanConfig
from ansible_collections.aviznetworks.sonic_fmcli.plugins.module_utils.network.sonic.argspecification.vlan import VlanArgs
...
def main():
    ...
    """
    module = AnsibleModule(argument_spec=VlanArgs.argument_spec, supports_check_mode=True)
